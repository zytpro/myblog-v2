...
<!--        <el-button type="primary" @click="applyFilter">搜索</el-button>-->
    ...
    <!-- 表格展示 -->
    <el-table :data="filteredCategories" border style="margin-top: 20px; width: 600px;">
...
const filteredCategories = computed(() => {
  return categories.filter((category) => {
    const matchName =
        !search.name || category.name.includes(search.name.trim());
    const matchStatus =
        search.status === null || category.status === search.status;
...
// 应用过滤条件
const applyFilter = () => {
  // 使用 Vue 的响应式特性，实时更新 `filteredCategories`
};
...
const saveCategory = async () => {
  try {
    if (form.id === null) {
      const response = await request.post("/categories", form);
      if (response.code === 0) {
      ...
      const response = await request.put(`/categories`, form);
      if (response.code === 0) {
...
const fetchCategories = async () => {
  try {
    ...
    if (Array.isArray(response)) {
      categories.splice(0, categories.length, ...response);
    }
  } catch (error) {
